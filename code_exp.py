...
mse_val = ((predictions_val - y_val) ** 2).mean().item()
...
mse_test = ((predictions_test - y_test) ** 2).mean().item()
...
# Highlighting the predicted values corresponding to -100s from validation and test sets
mask_val = y_val == -100
...
ax.scatter(predictions_val[mask_val].numpy(),
           ...
           color='blue', label='Predicted -100 Values (Validation Set)')
...
phi_train = np.array([[rbf(x.numpy(), c.numpy(), sigma)
                     for c in centers] for x in x_train])
phi_val = np.array([[rbf(x.numpy(), c.numpy(), sigma)
                   for c in centers] for x in x_val])
phi_test = np.array([[rbf(x.numpy(), c.numpy(), sigma) for c in centers]
...
mse_val_rbf = ((predictions_val_rbf - y_val) ** 2).mean().item()
rmse_val_rbf = math.sqrt(mse_val_rbf)
mse_test_rbf = (
    (predictions_test_rbf - negative_100_tensor[:, 2]) ** 2).mean().item()
rmse_test_rbf = math.sqrt(mse_test_rbf)
